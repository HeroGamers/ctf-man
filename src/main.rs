...
fn main() -> Result<()> {
    ...
    if args.len() == 3 {
        ...
        create_challenge(&config, category, challenge)?;
        return Ok(());
    } else if args.len() == 1 {
        ...
        if let Some(ref ctf_dir) = config.default_ctf_directory {
            templates::ensure_templates_directory(ctf_dir)?;
        }
...
fn run_tui(config: &mut Config) -> Result<()> {
                            ...
                            Action::RefreshFromAPI => {
                                ...
                                thread::spawn(move || {
                                    ...
                                    let result = (|| -> Result<usize> {
                                        let conn = db::schema::init_database(&db_path_clone)?;
...
fn perform_sync(db: &Database, config: &Config) -> Result<crate::fs::SyncResult> {
...
fn create_challenge(config: &Config, category: &str, challenge_name: &str) -> Result<()> {
    ...
    let challenge = db::models::Challenge {
        id: None,
        ctf_id,
